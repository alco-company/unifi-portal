...
class Admin::NasTest < ApplicationSystemTestCase
  ...
  test "visiting the index" do
    ...
    assert_selector "h1", text: "Network Access Points"
  end

  test "should create nas" do
    visit admin_tenant_site_nas_index_path(@tenant, @site)
    # Open actions dropdown (button containing sr-only span "Open nas actions")
    find("button span.sr-only", text: "Open nas actions").find(:xpath, "..").click
    click_on "New AP"
    ...
    fill_in "Nas type", with: @nas.nas_type
    click_on "Create"
  ...
  test "should destroy Nas" do
    visit admin_tenant_site_nas_path(@tenant, @site, @nas)
    find('[data-testid="nas-delete-trigger"]').click
    within("dialog") do
      assert_text "Confirm deletion"
      find('[data-testid="nas-delete-confirm"]').click
    end
